...
export default {
  ...
  methods: {
    async fetchReservations() {
      ...
      await fetch("https://localhost:8443/users/" + localStorage.getItem("userId"), addSectionOptions)
    ...
    async deleteReservation() {
      ...
      await fetch("https://localhost:8443/reservations/" + this.reservations[this.selectedIndex].reservationId, addSectionOptions)
          .then((response) => response.json())
          //Then with the data from the response in JSON...
          .then(data => {
            if (data.result) {
